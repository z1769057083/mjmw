  ...
  <div class="confirm-wrap">
    ...
    <div class="main" ref="scroll_hook">
      <div class="scroll-wrap">
        <div class="zw-50" v-show="zwFlag"></div>
        <!-- 无地址 -->
        ...
        <div class="com song-info">
          ...
          <router-link tag="div" class="right" :to="{name: 'dispatch'}" v-else>
            <span class="txt" v-if="isTips">
              运费 <s class="color">{{express > 0 ? '￥' + express + '+运费到付' : '运费到付'}}</s>
            </span>
        ...
        <div class="com song-msg">
          ...
          <input class="msg" type="text" v-model="note" maxlength="50" placeholder=" 选填，给商家留言（50字以内）" @click.stop>
        </div>
        <div class="cost-list">
    ...
    <div class="foot-wrap">
      <div class="left" v-if="(total - mjb + express * 1) == 0">合计：<i class="ic_mj"></i>{{mjb.toFixed(2)}}</div>
      <div class="left" v-else>合计：￥{{(total - mjb + express * 1).toFixed(2)}}</div>
    ...
    <div class="password-mark" v-show="passFlag">
      <div class="alert">
        ...
        <div class="cont">
          ...
          <div class="zw-5"></div>
          <router-link class="btn btn-setting" :to="{name: !isBindPhone ? 'phoneBind' : 'paySetting'}">设置密码</router-link>
  ...
  export default {
    ...
    methods: {
      ...
      goingPay() {
        ...
        setLocal('_deliveryWay', this.deliveryWay);
        setLocal('_pay', pay);
        if (this.addressObj.id == 0) {
          // 下订单
                    ...
                    if (res.data.amount > 0) {
                      this.$http.post(api.payMoney, qs.stringify({
                        ...
                        if (res.err_code == 200) {
                          ...
                          this.$router.replace({name: 'confirmID', query: {id: getLocal('_order_id')}});
                        }
                      });
                    } else {
                      console.log('全额美家币支付成功');
                      this.$router.replace({name: 'paySuccess'});
                    }
                  } else {
                    this.$router.replace({name: 'payError'});
                  }
                });
              }
            } else {
