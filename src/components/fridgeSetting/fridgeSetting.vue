<template>
  <div class="fridgeSetting-wrap">
    ...
    <div class="main">
      <!-- 温度设置 -->
      <div class="temp-wrap" ref="scroll_hook" v-show="curShow">
      ...
      <!-- 屏保设置 -->
      <div class="pic-wrap" ref="scroll2_hook" v-show="!curShow">
        <div class="scroll-wrap">
          <div class="pic-list" v-if="photos.length">
            <div class="item" v-for="(item, index) in photos" @touchstart="show" @touchend="clearShow">
              <div class="item-box" @click="showImg(index, photos)">
                <div class="img-box">
                  <img class="img preview-img" :src="item.photo">
                  <div class="img-mark"></div>
                  <div class="del-box" v-show="delFlag" @click.stop="hide">
                    <i class="icon" @click.stop="delImg(item, index)"></i>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div class="setting-wrap">
        <div class="tab-bar" v-if="!curShow">
          <div class="item left" :class="{active: screenSaver == 2}" @click="showTime">
            <i class="icon"></i>
            <span class="txt">显示时钟</span>
          </div>
          <div class="item center" :class="{active: screenSaver == 1}" @click="showPhoto">
            <i class="icon"></i>
            <span class="txt">播放相册</span>
          </div>
          <div class="item right" @click="uploadImg">
            <i class="icon"></i>
    ...
    methods: {
      ...
      uploadImg() {
        ...
        _this.$wechat.chooseImage({
          ...
          success(res) {
            // 返回选定照片的本地ID列表
            let localIds = res.localIds; // 返回的是一个数组
            for (let i = 0; i < localIds.length; i++) {
              _this.$wechat.uploadImage({
                ...
                success(res) {
                  ...
                  arr.push(serverId);
                  if (arr.length == localIds.length) {
                    _this.$http.post(api.addFridgePhoto, qs.stringify({
                      serverIds: arr.join('|')
                    })).then((res) => {
                      res = res.data;
                      if (res.err_code == 200) {
                        if (i == localIds.length - 1) {
                          _this.initPhoto(true);
                        }
                      }
                    }).catch((err) => {
                      console.log(err);
      ...
      delImg(item, index) {
        this.$http.post(api.deleteFridgePhoto, qs.stringify({
          id: item.id
        })).then((res) => {
          res = res.data;
          if (res.err_code == 200) {
      ...
      showImg(index) {
        if (!this.delFlag) {
          this.$preview.open(index, this.photos);
        }
      },
      showTime() {
        let type = (this.screenSaver == 0 || this.screenSaver == 1) ? 2 : 0;
        this.$http.post(api.fridgeScreenSet, qs.stringify({
          type: type
        })).then((res) => {
          res = res.data;
          if (res.err_code == 200) {
            this.screenSaver = type;
          }
        });
      },
      showPhoto() {
        let type = (this.screenSaver == 0 || this.screenSaver == 2) ? 1 : 0;
        this.$http.post(api.fridgeScreenSet, qs.stringify({
          type: type
        })).then((res) => {
          res = res.data;
      ...
      changeState(e) {
        ...
        if (index == 1) {
          ...
          if (this.fastCool == 0) {
            this.ajaxData({
              fastCool: 1,
              lower: (this.smart == 1 ? this.fridge.lower : this.lower),
              smart: '0',
              upper: 2
            }, () => {
              this.fastCool = 1;
          ...
          if (this.fastFrozen == 0) {
            this.ajaxData({
              fastFrozen: 1,
              upper: (this.smart == 1 ? this.fridge.upper : this.upper),
              smart: '0',
              lower: -24
            }, () => {
              this.fastFrozen = 1;
          ...
          if (this.lock == 0) {
            this.ajaxData({lock: 1}, () => {
              this.lock = 1;
            }, true);
          } else {
            this.ajaxData({lock: '0'}, () => {
              this.lock = 0;
            }, true);
          }
        }
      },
      ajaxData(options, callback, flag) {
        if (this.lock == 1 && !flag) {
          this.warning = !this.warning;
          this.tips = '冰箱锁定状态，请解除锁定';
          return;
      ...
      initPhoto(uploadFlag) {
        this.$http.post(api.fridgePhotoList, qs.stringify({
          page: (uploadFlag ? 1 : this.page)
        })).then((res) => {
          res = res.data;
          if (res.err_code == 200) {
            if (uploadFlag) {
            ...
            this.photos = (uploadFlag ? res.data.photos_url : this.photos.concat(res.data.photos_url));
      ...
      initScroll() {
        this.$nextTick(() => {
          ...
          if (!this.scroll2) {
            this.scroll2 = new BScroll(this.$refs.scroll2_hook, {
              click: true,
              probeType: 3
            });
            // 记录最大高度
            this.maxHeight = this.scroll2.maxScrollY;
            // 上拉加载
