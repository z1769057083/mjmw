<template>
  <div class="paySettingEdit-wrap">
      ...
      <div class="scroll-wrap">
        ...
        <div class="alert alert-code" v-if="step == 1">
          ...
          <div class="cont">
            ...
            <a class="btn btn-sure" @click="toValidate">确认</a>
          </div>
        </div>
        <!-- 填写密码 -->
        <div class="alert alert-password" v-else-if="step == 2">
          <div class="title">
            <i class="icon"></i>
          </div>
          <div class="cont">
            <div class="zw-10"></div>
            <div class="btn">
              <input class="input" v-model="pass" type="password" placeholder="请输入6位数字支付密码" ref="pass">
            </div>
            <div class="zw-10"></div>
            <div class="btn">
              <input class="input" v-model="pass2" type="password" placeholder="请再次输入密码" ref="pass2">
            </div>
    ...
    <!-- 成功框 -->
    <vtoastw v-show="success">
      <div slot="txt" class="txt">支付密码修改成功</div>
...
<script>
  ...
  export default {
    ...
    methods: {
      getCode() {
        this.toBlur();
        this.$http.post(api.reSetPayPass, qs.stringify({
          step: 1,
          mobile: this.phone
        })).then((res) => {
          res = res.data;
