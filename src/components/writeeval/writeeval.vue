          ...
          <div class="ord_lsit">
            <dl class="main_lsit" v-show="evall">
              <dd v-for="(item, index) in stars" class="temp">
                ...
                <div class="item_star">
                  <div class="aga_left">
                    <div class="left_in" :class="{active : atc}" @click="chec()">
    ...
    <div class="norele" v-show="rece_rele">
      <div class="rele_box">
        ...
        <div class="more_box">
          ...
          <div class="more_btn">
            <a class="more_sy fo53" @click="$router.back()">不保存</a>
          </div>
          <div class="more_btn">
...
//                  let serverId = res.serverId; // 返回图片的服务器端ID
        ...
        this.$http.post(api.commentCenter, qs.stringify({
          ...
          if (res.data.err_code == 200) {
            ...
            for (let i = 0; i < res.items.length; i++) {
              if (res.items[i].item_id == this.item_id) {
                this.stars.push(res.items[i]);
              }
            }
          } else {
            console.log('数据加载失败');
      ...
      // 添加商品评价接口
      _addGoods(flag) {
        this.$http.post(api.addComment, qs.stringify({
          order_id: this.order_id,
          item_id: this.item_id,
          rate: this.sign,
          comment: this.text,
